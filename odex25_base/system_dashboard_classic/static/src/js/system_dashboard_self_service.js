odoo.define('system_dashboard_classic.dashboard_self_services', function(require) {
                        ...
                        if (result.cards !== undefined && result.cards.length > 0) {
                            $.each(result.cards, function(index) {
                                if (result.cards[index].type == "selfs") {
                                    ...
                                    $(card).find('.card-body .box-1 h4').html(_t(result.cards[index].name));
                                    // appending cards
                            ...
                            self.$el.find('div.card3 .box-2').on('click', function(event) {
                                ...
                                //var regexPattern = /\[([^\]]+)\]/;
                                //var regexPattern2 = /(\w+)(?=,)/;
                                //context = context.replace(regexPattern, '["$1"]');
                                //context = context.replace(regexPattern2, '"$1"');
    ...
    core.action_registry.add('system_dashboard_classic.dashboard_self_services', SystemDashboardView);
...
function buildCardZ(data, index) {
    ...
    $(box2).addClass('col-md-12 col-sm-12 col-xs-12 box-2').attr('data-model', data.model).attr('data-form-view', data.form_view).attr('data-list-view', data.list_view).attr('data-name', data.name_english).attr('data-context', context);
    //box 1
    ...
    if ($('body').hasClass('o_rtl') === true) {
        $(box2_i).addClass('mdi mdi-plus').attr('title', 'أضف' + data.name + ' جديد');
        $(span_i).html('إضافة جديدة');
    } else {
        $(box2_i).addClass('mdi mdi-plus').attr('title', 'Create New ' + data.name);
        $(span_i).html('Add New');
    }
