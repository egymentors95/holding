...
class PurchasePlanning(models.Model):
    ...
    order_line = fields.One2many('purchase.order.line', 'purchase_plan_id', string='Order Lines',
    ...
    plan_start_date = fields.Date(string='Start Date', states=READONLY_STATES, copy=False,
                                  help="Scheduled date to start the purchase order.")
    plan_end_date = fields.Date(string='End Date', states=READONLY_STATES, copy=False,
                                help="Scheduled date to finish the purchase order.")
    no_of_months = fields.Integer(string='No of Months', states=READONLY_STATES, copy=False,
                                  help="Number of months for the purchase order.", compute='_compute_no_of_months',
                                  store=True)
    ...
    def create(self, vals):
        ...
        if vals.get('name', 'New') == 'New':
            ...
            vals['name'] = self.env['ir.sequence'].next_by_code('purchase.planning') or '/'
        vals, partner_vals = self._write_partner_values(vals)
        res = super(PurchasePlanning, self_comp).create(vals)
        if partner_vals:
    ...
    def _compute_no_of_months(self):
        for order in self:
            if order.plan_start_date and order.plan_end_date:
                start = min(order.plan_start_date, order.plan_end_date)
                end = max(order.plan_start_date, order.plan_end_date)

                days = (end - start).days + 1
                print("days", days)
                order.no_of_months = round(days / 30)
                print("no_of_months", order.no_of_months)
            else:
                order.no_of_months = 0
    ...
    def _compute_access_url(self):
        super(PurchasePlanning, self)._compute_access_url()
        for order in self:
    ...
    def copy(self, default=None):
        ...
        new_po = super(PurchasePlanning, self).copy(default=default)
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        """
        result = super(PurchasePlanning, self).onchange(values, field_name, field_onchange)
    ...
    def _track_subtype(self, init_values):
        ...
        return super(PurchasePlanning, self)._track_subtype(init_values)
    ...
    def message_post(self, **kwargs):
        ...
        return super(PurchasePlanning, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)
    ...
    def button_confirm(self):
        for order in self:
            return order.write({'state': 'purchase'})
    ...
    def retrieve_dashboard(self):
        ...
        self.env.cr.execute(query,
                            (one_week_ago, tuple(list_old_value_char), tuple(list_new_value_char), self.env.company.id))
    ...
    def confirm_reminder_mail(self, confirmed_date=False):
        for order in self:
            if order.state in ['purchase', 'done'] and not order.mail_reminder_confirmed:
                ...
                order.message_post(
                    body=_("%(name)s confirmed the receipt will take place on %(date)s.", name=order.partner_id.name,
                           date=date))
    ...
    def _confirm_reception_mail(self):
        for order in self:
            if order.state in ['purchase', 'done'] and not order.mail_reception_confirmed:
                ...
                order.message_post(
                    body=_("The order receipt has been acknowledged by %(name)s.", name=order.partner_id.name))
    ...
    def _update_update_date_activity(self, updated_dates, activity):
        for line, date in updated_dates:
            activity.note += _('<p> &nbsp; - %s from %s to %s </p>') % (line.product_id.display_name,
                                                                        line.date_planned.date(), date.date())
    ...
    def action_quotation(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'context': {
                "default_purchase_plan_id": self.id,
...
class PurchaseOrder(models.Model):
    ...
    purchase_plan_id = fields.Many2one('purchase.planning', string='Purchase Plan', copy=False,
                                       help="Reference to the purchase plan if this order is generated from a purchase plan.")

    @api.constrains('order_line', 'date_order')
    def _check_quantities_with_plan(self):

        month = fields.Date.from_string(self.date_order).month
        month_name = calendar.month_name[month].lower()

        for line in self.order_line:
            planned_line = self.purchase_plan_id.order_line.filtered(lambda l: l.product_id == line.product_id)
            if not planned_line:
                continue

            planned_qty = getattr(planned_line, month_name, 0.0) or 0.0
            print("planned_qty", planned_qty)
            rfq_qty = line.product_qty

            if planned_qty == 0:
                channel = self.env['mail.channel'].channel_get(partners_to=[self.env.user.partner_id.id])['id']
                print("Channel ID:", channel)
                msg = self.env['mail.channel'].browse(channel).message_post(
                    body="The RFQ quantity is ZERO in the planned quantity.",
                ...
                return {
                    'warning': {
                        'title': _('Warning'),
                        'message': _("The RFQ quantity is ZERO in the planned quantity."),
                    }
                }
            elif rfq_qty < planned_qty:
                channel = self.env['mail.channel'].channel_get(partners_to=[self.env.user.partner_id.id])['id']
                print("Channel ID:", channel)
                msg = self.env['mail.channel'].browse(channel).message_post(
                    body="The RFQ quantity is SMALLER the planned quantity.",
                    message_type='comment',
                    subtype_xmlid='mail.mt_comment',
                )
                print("Message sent:", msg)
                return {
                    'warning': {
                        'title': _('Warning'),
                        'message': _("The RFQ quantity is SMALLER in the planned quantity."),
                    }
                }
                ...
                print("Channel ID:", channel)
                msg = self.env['mail.channel'].browse(channel).message_post(
                    body="The RFQ quantity is BIGGER the planned quantity.",
                    message_type='comment',
                    subtype_xmlid='mail.mt_comment',
                )
                print("Message sent:", msg)
                return {
                    ...
                    'params': {
                        'title': _("The RFQ quantity is BIGGER than the planned quantity."),
                        'type': 'warning',
                        'sticky': True,
...
class PurchaseOrderLine(models.Model):
    ...
    purchase_plan_id = fields.Many2one('purchase.planning', string='Purchase Plan')
    january = fields.Float(string='January', copy=False,
                           help="Quantity planned for January.")
    february = fields.Float(string='February', copy=False,
                            help="Quantity planned for February.")
    march = fields.Float(string='March', copy=False,
                         help="Quantity planned for March.")
    april = fields.Float(string='April', copy=False,
                         help="Quantity planned for April.")
    may = fields.Float(string='May', copy=False,
                       help="Quantity planned for May.")
    june = fields.Float(string='June', copy=False,
                        help="Quantity planned for June.")
    july = fields.Float(string='July', copy=False,
                        help="Quantity planned for July.")
    august = fields.Float(string='August', copy=False,
                          help="Quantity planned for August.")
    september = fields.Float(string='September', copy=False,
                             help="Quantity planned for September.")
    october = fields.Float(string='October', copy=False,
                           help="Quantity planned for October.")
    november = fields.Float(string='November', copy=False,
                            help="Quantity planned for November.")
    december = fields.Float(string='December', copy=False,
                            help="Quantity planned for December.")
    product_uom = fields.Many2one('uom.uom', related='product_id.uom_id', readonly=False)

    @api.onchange('january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october',
                  'november', 'december', 'price_subtotal')
    def _compute_product_qty(self):
        for line in self:
            if line.purchase_plan_id:
                line.product_qty = line.january + line.february + line.march + line.april + line.may + line.june + line.july + line.august + line.september + line.october + line.november + line.december
            else:
    ...
    def create(self, vals_list):
        ...
        for line in records:
            line._check_monthly_planning_qty()
        return records
    ...
    def _check_monthly_planning_qty(self):
        order = self.purchase_plan_id
        if not order.date_order or not self.product_id:
            return

        month = fields.Date.from_string(order.date_order).month
        month_name = calendar.month_name[month].lower()

        planned_qty = getattr(self, month_name, 0.0) or 0.0
        rfq_qty = self.product_qty

        if self.product_id and (rfq_qty <= 0 or rfq_qty != planned_qty):
            return {
                'warning': {
                    'title': _("Warning for %s") % self.product_id.display_name,
                    'message': _("RFQ qty (%s) differs from planned qty (%s) for %s.") %
                               (rfq_qty, planned_qty, month_name.title()),
                }
            }
