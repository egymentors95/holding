...
class SpreadsheetDocuments(SavepointCase):
    ...
    def test_spreadsheet_to_display_without_contrib(self):
        user = new_test_user(self.env, login="Jean", groups="documents.group_dms_user")
        with freeze_time('2020-02-02 18:00'):
    ...
    def test_spreadsheet_to_display_access_ir_rule(self):
        user = new_test_user(self.env, "Test user", groups='documents.group_dms_manager')

        model = self.env.ref('documents.model_documents_document')
        group = self.env.ref('documents.group_dms_manager')
    ...
    def test_user_right_own_template(self):
        user = new_test_user(self.env, "Test user", groups='documents.group_dms_user')
    ...
    def test_user_right_not_own_template(self):
        manager = new_test_user(self.env, "Test manager", groups='documents.group_dms_manager')
        user = new_test_user(self.env, "Test user", groups='documents.group_dms_user')
    ...
    def test_contributor_write_raw(self):
        ...
        user = new_test_user(self.env, "Test Manager", groups='documents.group_dms_manager')
